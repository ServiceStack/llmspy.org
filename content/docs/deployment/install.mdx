...
These set `VERBOSE=1` / `DEBUG=1` inside the container, which is what llms.py
...
The microphone is in your browser, so nothing needs to be passed into the
container. Add a speech-to-text key and restart:
...
See [Voice Input](/docs/features/voice-input) for local speech-to-text servers,
the other modes, and every setting.
